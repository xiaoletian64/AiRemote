...
新增 `usb_hid_release_all()`，清空 `s_kb_state`/`s_consumer_state`；已挂载时发键盘 `{1,0,0,0,0,0,0,0,0}` 和 Consumer `{2,0,0}` 空报告，未挂载时仍必须清状态。
...
将 HID 解析状态移到可清理作用域；`ESP_GATTC_DISCONNECT_EVT`、失败的 `ESP_GATTC_OPEN_EVT` 和下一次扫描前发布空报告后复位输入状态。队列投递失败记录 page、usage、down/up 并触发安全释放。
