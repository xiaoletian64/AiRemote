...
impl HidListener {
    pub fn new() -> Self {
        Self { running: Arc::new(Mutex::new(false)), thread_handle: None }
    }
    ...
    pub fn scan_known_devices() -> Result<Vec<String>, String> {
        ...
        Ok(api.device_list()
            ...
            .map(|d| format!(
                ...
                d.product_string().unwrap_or("未知"), d.vendor_id(), d.product_id(),
    ...
    where F: Fn(u16, bool) + Send + 'static,
    {
                        ...
                        match info.open_device(&api) {
                            Ok(device) => {
                                ...
                                diagnostics::record(&diagnostics, format!(
                                    ...
                                    info.product_id(), info.usage_page()));
                                    ...
                                    match device.read_timeout(&mut buf, 1000) {
                                        Ok(size) if size > 0 => {
                                            ...
                                            if is_release_report(data) {
                                                ...
                                                if usage != prev_usage {
                                                    if prev_usage != 0 { callback(prev_usage as u16, false); }
                                        ...
                                        Err(e) => {
                                            ...
                                            if msg.contains("拒绝访问") || msg.contains("Access") {
                                                ...
                                                diagnostics::record(&diagnostics, format!("HID 读错误: {}", msg));
                                            }
                                ...
                                if prev_usage != 0 { callback(prev_usage as u16, false); prev_usage = 0; }
                            }
                            Err(e) => {
                                let msg = e.to_string();
                                if msg.contains("拒绝访问") || msg.contains("Access") {
                                    ...
                                    diagnostics::record(&diagnostics, format!("HID 无法打开: {}", msg));
                                }
                            }
                        }
                    }
